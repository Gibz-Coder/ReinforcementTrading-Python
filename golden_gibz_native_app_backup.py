...
class GoldenGibzNativeApp:
    ...
    def create_trading_tab(self):
        ...
        # Trading log with split display (takes remaining space)
        ...
        self.signal_header.config(state=tk.NORMAL)
        self.insert_colored_text_to_widget(self.signal_header, "📊 Latest Signal Analysis\n", "info")
        self.insert_colored_text_to_widget(self.signal_header, "Waiting for first signal...\n", "warning")
        self.insert_colored_text_to_widget(self.signal_header, "=" * 50 + "\n", "info")
        self.signal_header.config(state=tk.DISABLED)  # Make it read-only
        
        # Separator line
        separator = tk.Frame(log_frame, height=2, bg=self.colors['accent'])
        separator.pack(fill=tk.X, pady=2)
        
        # Scrolling log section for continuous updates
        scroll_frame = tk.Frame(log_frame, bg=self.colors['text_bg'])
        scroll_frame.pack(fill=tk.BOTH, expand=True)
        
        self.trading_log = self.create_colorful_text_widget(scroll_frame, height=15, width=70)
                                    ...
                                    if hasattr(ea, 'execute_trade'):
                                        try:
                                            ...
                                            if trade_result:
                                                ...
                                                # Get specific error reason from EA
                                                if hasattr(ea, 'last_error') and ea.last_error:
                                                    self.log_queue.put(('trading', f"❌ {ea.last_error}"))
                                                else:
                                                    self.log_queue.put(('trading', f"❌ Trade execution failed - EA returned False"))
                                                
                                                # Check MT5 last error only if no specific EA error
                                                if not (hasattr(ea, 'last_error') and ea.last_error):
...
Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    def check_log_queue(self):
        """Check for new log messages with color support and route to appropriate sections"""
        try:
            while True:
                ...
                if log_type == 'trading':
                    # Check if this is a signal analysis message (should go to header)
                    if self.is_signal_analysis_message(message):
                        self.update_signal_header(message, color_tag)
                    # Status updates go to scrolling log
                        ...
                        self.trading_log.see(tk.END)
                    # Other trading messages go to scrolling log
                    else:
                        self.insert_colored_text_to_widget(self.trading_log, f"{timestamp} - ", "info")
                        self.insert_colored_text_to_widget(self.trading_log, f"{message}\n", color_tag)
                        self.trading_log.see(tk.END)
    ...
    def is_signal_analysis_message(self, message):
        """Check if message is part of signal analysis that should go to header"""
        signal_keywords = [
            "📈 Market Analysis",
    ...
    def update_signal_header(self, message, color_tag):
        """Update the fixed signal header section"""
        # Enable editing temporarily
        self.signal_header.config(state=tk.NORMAL)
        
        # If this is the start of a new analysis cycle, clear the header
        if "📈 Market Analysis" in message:
            self.signal_header.delete(1.0, tk.END)
        
        # Convert confidence to percentage format
        if "⚖️ Confidence:" in message and "0." in message:
            # Extract confidence value and convert to percentage
            import re
            confidence_match = re.search(r'Confidence:\s*(0\.\d+)', message)
            if confidence_match:
                confidence_val = float(confidence_match.group(1))
                percentage = int(confidence_val * 100)
                message = message.replace(confidence_match.group(0), f"Confidence: {percentage}%")
        
        # Add message to header
        self.insert_colored_text_to_widget(self.signal_header, f"{message}\n", color_tag)
        
        # Add separator after analysis is complete
        if "⏳ Waiting" in message and "for next analysis" in message:
            self.insert_colored_text_to_widget(self.signal_header, "=" * 50 + "\n", "info")
