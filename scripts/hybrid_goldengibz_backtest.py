#!/usr/bin/env python3
"""
Hybrid Golden Gibz AI-Enhanced Backtesting System - FINAL VERSION
================================================================
AI-Enhanced backtesting system combining technical analysis with AI model confirmation
- Technical Analysis: Primary signal generation (proven 61.7% win rate)
- AI Model: Confirmation layer for signal quality enhancement  
- Hybrid Logic: Only trades when both systems provide reasonable confidence
- Target Win Rate: 62%+ with optimized risk management
...
class HybridGoldenGibzBacktester:
    """AI-Enhanced Golden-Gibz backtesting system with hybrid intelligence."""
    
    def __init__(self, model_path=None, symbol="XAUUSD"):
        self.symbol = symbol
        self.model_path = model_path or "models/production/golden_gibz_wr100_ret+25_20251225_215251.zip"
        self.model = None
        self.data = {}
        self.results = {}
        ...
        print(f"{Fore.CYAN}{Style.BRIGHT}🎯 GOLDEN-GIBZ AI-ENHANCED BACKTESTER")
        print(f"{'='*70}")
        ...
        print(f"Target Win Rate: >65% (AI-Enhanced)")
        print(f"AI Integration: Technical + AI Confirmation")
        print(f"{'='*70}{Style.RESET_ALL}")
    
    def create_observation_for_ai(self, data, timestamp):
        ...
        try:
            ...
            idx = exec_data.index.get_indexer([timestamp], method='nearest')[0]
            if idx < 20:
                return np.zeros((20, 21), dtype=np.float32)
            
            # Get the last 20 bars of data
            recent_data = exec_data.iloc[idx-19:idx+1]
    ...
    def get_ai_prediction(self, data, timestamp):
        ...
        try:
            ...
            observation = self.create_observation_for_ai(data, timestamp)
    ...
    def analyze_enhanced_conditions(self, timestamp):
        ...
        try:
            ...
            # Enhanced signal determination with more aggressive criteria
            min_strength = 2.0  # REDUCED from 2.5 to allow even more signals
    ...
    def generate_enhanced_signal(self, timestamp):
        """Generate enhanced trading signal with AI model confirmation - HYBRID APPROACH."""
        try:
            # Step 1: Get technical analysis signal (our proven 61.7% system)
            conditions = self.analyze_enhanced_conditions(timestamp)
            if not conditions:
                return None
            
            # Step 2: Get AI model prediction as confirmation
            ai_prediction = self.get_ai_prediction(self.data, timestamp)
            ...
            # Step 4: AI-Technical Hybrid Decision Logic - MORE PERMISSIVE
            final_action = 0
            ...
            if ai_prediction and technical_action != 0:
                ...
                # Case 2: Technical says trade, AI says hold - MORE PERMISSIVE
                elif technical_action != 0 and ai_action == 0:
                    # Allow technical-only trades with lower requirements
                    if technical_confidence > 0.65:  # Lowered from 0.7
                        final_action = technical_action
                ...
                # Case 3: AI says trade, technical says hold - MORE PERMISSIVE
                elif technical_action == 0 and ai_action != 0:
                    # Allow AI-only trades with even lower requirements
                    if ai_confidence > 0.60 and conditions['signal_strength'] > 0.8:  # Further lowered
                ...
                # Fallback to technical analysis only if AI fails - VERY PERMISSIVE
                if technical_confidence > 0.60:  # Lowered from 0.65
                    final_action = technical_action
            ...
            print(f"⚠️ Enhanced signal generation error: {e}")
            return None
    
    def run_backtest(self, start_date=None, end_date=None):
        """Run the final optimized backtest."""
        print(f"\n{Fore.YELLOW}{Style.BRIGHT}🚀 STARTING GOLDEN-GIBZ BACKTEST...")
        print(f"{'='*70}{Style.RESET_ALL}")
        
        if not self.model or self.execution_tf not in self.data:
        ...
        print(f"📊 Fixed lot size: {self.fixed_lot_size} lots")
        print(f"📊 Risk-Reward: 1:{self.rr_ratio}")
        print(f"📊 Min confidence: {self.min_confidence}")  # Will show 0.65
        ...
        print(f"\n⏳ Processing {total_bars:,} bars with enhanced filtering...")
        ...
        for i in range(50, len(df)):
            ...
            if i % 4 == 0 and len(positions) < self.max_positions:
                signal = self.generate_enhanced_signal(current_time)
                signals_generated += 1
                ...
                if signal and signal['confidence'] >= self.min_confidence:
                    ...
                    if action == 1:  # LONG
                        ...
                        positions.append({
                            ...
                            'signal_type': signal.get('signal_type', 'UNKNOWN')  # Add signal type
                        })
            ...
            for j, pos in enumerate(positions):
                ...
                if exit_trade:
                    ...
                    trades.append({
                        ...
                        'signal_type': pos.get('signal_type', 'UNKNOWN')  # Add signal type
                    })
        ...
        print(f"\n✅ Golden-Gibz backtest completed!")
    ...
    def analyze_results(self):
        ...
        print(f"\n{Fore.CYAN}{Style.BRIGHT}📊 GOLDEN-GIBZ RESULTS ANALYSIS")
        print(f"{'='*70}{Style.RESET_ALL}")
        ...
        if win_rate >= 65:
            rating = f"{Fore.GREEN}🚀 OUTSTANDING (AI-Enhanced)"
        elif win_rate >= 60:
        ...
        if len(trades_df) > 0 and 'signal_type' in trades_df.columns:
            print(f"\n{Fore.CYAN}🤖 AI INTEGRATION STATISTICS")
            print(f"{'─'*50}")
            
            signal_types = trades_df['signal_type'].value_counts()
            for signal_type, count in signal_types.items():
                percentage = (count / len(trades_df)) * 100
                print(f"{signal_type}: {count} trades ({percentage:.1f}%)")
            
            # Win rates by signal type
            print(f"\n{Fore.MAGENTA}📊 WIN RATES BY SIGNAL TYPE")
            print(f"{'─'*50}")
            for signal_type in signal_types.index:
                ...
                type_win_rate = (type_wins / len(type_trades)) * 100 if len(type_trades) > 0 else 0
                print(f"{signal_type}: {type_win_rate:.1f}% ({type_wins}/{len(type_trades)})")
    ...
    def save_results(self, filename=None):
        ...
        if filename is None:
            ...
            filename = f'backtest_results/golden_gibz_results_{timestamp}.json'
...
def main():
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}🤖 GOLDEN-GIBZ AI-ENHANCED BACKTESTING")
    print(f"{'='*70}{Style.RESET_ALL}")
    
    # Initialize backtester
    backtester = HybridGoldenGibzBacktester()
    ...
    if results:
        ...
        print(f"\n{Fore.GREEN}{Style.BRIGHT}🚀 GOLDEN-GIBZ AI-ENHANCED BACKTESTING COMPLETED!")
        print(f"Hybrid system combining technical analysis with AI model confirmation.")
